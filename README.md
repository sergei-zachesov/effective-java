...
так
и не обновляет никакое состояние. Для этого нужно добиться, что бы функциональные объекты потоковых операций, были без
побочных эффектов. А не маскировать интервальный код, под потоковое вычесление.

*Входные данные не должные зависить от изменяемого состояния указывает* - из лямбда-выражения можно прочесть только
...
## 7.7 Будьте внимательны при параллелизации потоков (Item 48)